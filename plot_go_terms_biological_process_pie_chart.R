...
z <- y[c(1:13)]
new_row <- data.table(GO_term = "other", N = 2708, label="other")
z <- rbind(z, new_row)
...
z[GO_term %in% c("GO:0000079", "GO:0045892"), srt := 310]

# make pie chart and save to file
png("go_terms_biological_process.png")
pie(z$N, labels=z$label, main="Biological Process", cex=1, srt=z$srt)
dev.off()