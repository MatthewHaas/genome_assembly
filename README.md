...
## Figure 2C
**Figure 2C** was created using the MCscan program ([run_jcvi.sh](run_jcvi.sh)). I called the program JCVI in the script name rather than MCscan because the scripts are found in a directory called jcvi in the GitHub repository for the MCscan code. You should use the [seqids](synteny_figures/main_mcscan_seqids) file in conjuntion with [run_jcvi.sh](run_jcvi.sh), but note that you should just call is seqids and sequester it away from the other seqids file used for the comparison with _Z. latifolia_. I changed the name here only so that the two files would not have the same name which would overwrite the other.
